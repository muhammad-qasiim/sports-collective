    ...
    return (
        <>
            <main className={`fixed bottom-0 right-0 left-0 z-10 transition-all ${toggleSidebar ? 'sidebar-container' : 'bg-transparent pointer-events-none'}`} style={{ marginLeft: '0px', top: '4.4rem' }}>
                <aside className={`${toggleSidebar ? 'sidebar-open' : 'sidebar-close'} flex justify-between gap-12 flex-col`}>
                    <ul className="flex pt-40 flex-col items-center mt-4 ">
                        {
                            Navigation?.map(item => (
                                <li className='mb-12' onClick={() => setToggleSidebar(false)}>
                                    <Link to={item?.path}>
                                        <span className={`relative text-sm md:text-xl border-b-2 border-transparent cursor-pointer transition-all text-gray-500 hover:text-gray-800`}>
                                            {item?.placeholder}
                                        </span>
                                    </Link>
                                </li>
                    ...
                    <section className='gGngoX'>
                        <Link to="/sign-up"> <p onClick={() => setToggleSidebar(false)} className="relative text-center mb-22 text-sm md:text-xl border-b-2 border-transparent cursor-pointer transition-all text-gray-500 hover:text-gray-800" >CREATE ACCOUNT</p> </Link>
